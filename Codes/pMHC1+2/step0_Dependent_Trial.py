...
def LinearProblem(Meta_data, random_combination, pdb_id, All_results) :
    ...
    target_data = Meta_data[Meta_data['PDB_ID'] == pdb_id]
    answer_data = target_data.query("Is_Answer == 1")
    other_data = target_data.query("Is_Answer != 1")
...
def Dependent_Trial(Meta_data, random_combination) :
    ...
    pdb_ids = Meta_data["PDB_ID"].tolist() 
